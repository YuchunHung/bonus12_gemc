...
map<string, double> rtpc_HitProcess :: integrateDgt(MHit* aHit, int hitn)
{
  ...
  //int trkID = aHit->GetTId(); //cout<<"kp:    trkID = "<< trkID <<endl; //kp: 2/22/21
  //double rnd4Tshift = 0.0;
  ...
  //kp: I am replacing this if-block with the following one //kp: 2/22/21
 
  /*
  int iTid = 0, tidGt2 = 0;
  if(trkID != timeShift_map.cend()->first) {
    if(trkID < 3) timeShift_map[trkID] = 0.0;
    else if (timeShift_map[trkID]==0.0) { //if ( trkID > 2 && !(timeShift_map[trkID]==0.0) ) { // && !fabs(timeShift_map[trkID]) < 8000.) {
      
      for (auto iter = timeShift_map.begin(); iter != timeShift_map.end(); ++iter){
        iTid = iter->first;  if (iTid > 2) {tidGt2 = iTid; timeShift_map[trkID] = iter->second;  break; }
      }

      if ( tidGt2 == 0) //I
	timeShift_map[trkID] = G4RandFlat::shoot(-8000.,8000.);
    }
  } 
  */
  ...
  if(tInfos.eTot > 0)
      ...
      for(unsigned int s=0; s<tInfos.nsteps; s++)
	{
	  ...
	  */
	  ...
	  //cout<<"kp: tid = "<< aHit->GetTId() <<" ts = "<< shift_t <<endl;
	  // NO time shift
	  //shift_t = 0.0;
